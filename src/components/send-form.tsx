...
import { Copy, Loader2, Send, UploadCloud, X } from 'lucide-react';
...
export function SendForm() {
  ...
  const handleCopy = () => {
    if (formState?.id) {
      navigator.clipboard.writeText(`${window.location.origin}#${formState.id}`);
      toast({
        ...
        <Card className="shadow-lg">
            ...
            <CardContent>
                <div className="flex items-center space-x-2">
                    ...
                    <Button size="icon" className="h-12 w-12" onClick={handleCopy}><Copy className="h-6 w-6" /></Button>
                </div>
                <Button variant="link" className="px-0 mt-4" onClick={() => formState.id = undefined}>Share something else</Button>
            </CardContent>
        </Card>
    )
  }
          ...
          >
            {file ? (
                <div className="text-center">
                    ...
                    <Button variant="ghost" size="icon" className="absolute top-2 right-2 text-muted-foreground" onClick={() => setFile(null)}>
                        <X className="h-4 w-4" />
                    </Button>
                </div>
            ) : (
                <>
                <div className={cn("absolute inset-0 bg-card/80 backdrop-blur-sm z-10 flex flex-col justify-center items-center transition-opacity", isDragging ? 'opacity-100' : 'opacity-0 pointer-events-none')}>
